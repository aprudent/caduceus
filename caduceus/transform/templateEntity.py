...
class CaduceusTemplateResults:
    ...
    def __init__(self, tmplPath):
        ...
        self._executionStartTime = time.clock()
    ...
    def getExecutionTime(self):
        return int(self._executionTime * 1000)
    
    def end(self):
        self._executionTime = time.clock() - self._executionStartTime
...
class CaduceusTemplateEntity:
    ...
    def endTag(self, tag):
        ...
        while not token._matchTag(tag):
            if token._parent:
                ...
                # The opening tag doesn't exist !
                return None
        
        token._isClosedTag = True
    ...
    def findFirstTag(self, tag):
        if self._matchTag(tag):
            return self
        
        for token in self._childs:
            tokenFound = token.findFirstTag(tag)
            if tokenFound:
                return tokenFound
